...
public class RedTeleOp extends NextFTCOpMode {
    ...
    public static boolean intakeMode1;

    // Pass-through point relative to goal (inches)
    public static double passThroughRadiusIn = 18.0;  // back from goal along +Y
    public static double passThroughHeightIn = 60.0;  // height of pass-through
...
//        }
        ...
        if (!automatedDrive) {
            if (autoaim) {
                futureX = pose.getX() - robotVx * estTOF;
                futureY = pose.getY() - robotVy * estTOF;

                double futuredX = 132.0 - futureX;
...
//                rawError = angleWrap();
        ...
        telemetry.addData("Distance", "%.1f in", goalDistIn);
        telemetry.addData("Calculated RPM", "%.0f", baseRPM);
        ...
        telemetry.addData("futureX", futureX);
        telemetry.addData("futureY", futureY);
